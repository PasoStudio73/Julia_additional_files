...
sr = X.audio.sr
x_length = size(X.audio.data, 1)

wavelet = :morlet # :morse, :morlet, :bump
morse_params = (3, 20)
vpo = 20
boundary = :reflection # :reflection, :periodic
freq_range = :pagliarini # :full, :pagliarini

# get_stft!(X, X.audio)
# get_fbank!(X, X.stft)
get_cwt_fb!(X, X.audio)
...
#     if freq_range == :full
#         minscale = omegaC / pi
#     elseif freq_range == :pagliarini
#         minscale = log2(40) # log(40)
#     else
#         error("Frequency range $freq_range not supported.")
#     end
...
#     scales, center_freq = cwt_scales(wavelet, x_length, gamma_beta, vpo, freq_range)
...
#     freq_range::Symbol = :pagliarini
# )
#     fbank, wcf, signal_pad = _get_cwt_fb(rack.audio.sr, size(rack.audio.data, 1); wavelet, morse_params, vpo, boundary, freq_range)
