...
function _get_f0(
	...
	elseif method == :srh
		...
		# residual = zeros(eltype(x), x_length)
		# for i in 1:hops
		# 	idx1 = 1 + nhop * (i - 1)
		# 	idx2 = min(nwin + nhop * (i - 1), x_length)
		# 	idx = idx1:idx2
		# 	residual[idx] .+= invrt[i][1:min(length(idx), nwin)]
		# end

		# residual_buff = zeros(eltype(x), nwin, hops)
		# for hop in 1:hops
		# 	idx1 = 1 + nhop * (hop - 1)
		# 	idx2 = min(nwin + nhop * (hop - 1), x_length)
		# 	temp = residual[idx1:idx2]
			
		# 	residual_buff[1:min(nwin, length(temp)), hop] = temp
		# end
		...
		# 	idx2 = min(nwin + nhop * (hop - 1), x_length)
		# 	temp = residual[idx1:idx2]
		# 	[temp; zeros(eltype(x), max(0, nwin - length(temp)))]
		# end for hop in 1:hops)
				
		# win = _get_window(:blackman, nwin, true)
		# residual_buff = _get_wframes(residual_buff, win)
...
q2=_get_f0(x, sr; x_length=x_length, nwin=nwin, noverlap=noverlap, method=method, freqrange=freqrange)
