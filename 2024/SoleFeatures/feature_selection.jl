...
abstract type AbstractIdentityFilter{T<:AbstractLimiter} <: SoleFeatures.UnivariateFilterBased{T} end

struct IdentityFilter{T <: SoleFeatures.IdentityLimiter} <: AbstractIdentityFilter{T}
    limiter::SoleFeatures.IdentityLimiter
    # parameters

    function IdentityFilter()
        new{SoleFeatures.IdentityLimiter}(SoleFeatures.IdentityLimiter())
    end
end

SoleFeatures.is_supervised(::AbstractIdentityFilter) = true
SoleFeatures.is_unsupervised(::AbstractIdentityFilter) = true
...
"""
struct PercentageLimiter <: SoleFeatures.AbstractLimiter
    perc::Float64
    rev::Bool

    function PercentageLimiter(perc::AbstractFloat, rev::Bool)
        (perc <= 0 || perc >= 1.0) && throw(DomainError(perc, "`perc` must be > 0 and < 1"))
        return new(perc, rev)
    end
...
function SoleFeatures.limit(scores::AbstractVector{<:Real}, l::PercentageLimiter)
...
function _normalize_dataset!(
    ...
    for group in SoleFeatures.Experimental.groupby(es, group)
...
function feature_selection(
    ...
    ex_windows::AbstractVector = [ SoleFeatures.Experimental.FixedNumMovingWindows(5, 0.05)... ],
    ...
    newX = begin
        ...
        extract = SoleFeatures.Experimental.extract
        # TODO: this Float64 is a strong assumption!
        Float64.(@scache_if !isnothing(ced) "dse" ced extract(X, extract_tuples))
    end

    # groups_separator = "@@@"
    if groups_separator != SoleFeatures.Experimental._SEPARATOR
        rename!(x -> replace(x, SoleFeatures.Experimental._SEPARATOR => groups_separator), newX)
    end
