...
include("scanner.jl")
include("datasets/wav2stft_time_series.jl")
include("lib.jl")
...
function apply_tree_to_datasets_wavs(
        ...
        tree                                  :: DTree{S},
        dataset                               :: GenericModalDataset,
        wav_paths                             :: AbstractVector{<:AbstractString},
        labels                                :: AbstractVector{<:S};
        # MODE OPTIONS
        filter_type                           :: Symbol                           = :dynamic,  # :static
        mode                                  :: Symbol                           = :bandpass, # :emphasize
        # OUTPUT OPTIONS
        only_files                            :: AbstractVector{<:AbstractString} = Vector{String}(),
        files_already_generated               :: Bool                             = false,
        use_original_dataset_filesystem_tree  :: Bool                             = false,
        destination_dir                       :: AbstractString                   = "filtering-results/filtered",
        remove_from_path                      :: AbstractString                   = "",
        save_single_band_tracks               :: Bool                             = true,
        generate_json                         :: Bool                             = true,
        json_file_name                        :: AbstractString                   = "files.json",
        verbose                               :: Bool                             = false,
        draw_worlds_on_file                   :: Bool                             = true,
        # WAV POST-PROCESS
        postprocess_wavs                      :: AbstractVector{<:Function}       = [ trim_wav!,      normalize! ],
        ...
        draw_anim_for_instances               :: AbstractVector{<:Integer}        = Vector{Int64}(),
        ...
        wintime                               :: Real                             = 0.025,
        steptime                              :: Real                             = 0.01,
        movingaverage_size                    :: Integer                          = 1,
        movingaverage_step                    :: Integer                          = 1
    ) where {S}
    ...
    _n_samples = nsamples(dataset)

    println()
    println("Applying tree $(tree_hash):")
    print_tree(tree)
    println()
    print_apply(tree, dataset, labels)
    println()
    ...
    @assert _n_samples == length(wav_paths) "dataset and wav_paths length mismatch! $(_n_samples) != $(length(wav_paths))"
    @assert _n_samples == length(labels) "dataset and labels length mismatch! $(_n_samples) != $(length(labels))"
    ...
    if dataset isa MultiFrameModalDataset
        @assert nframes(dataset) == 1 "MultiFrameModalDataset with more than one frame is still not supported! nframes(dataset): $(nframes(dataset))"
    end

    json_archive = Dict{String, Vector{String}}()

    results = Vector{InstancePathInTree}(undef, _n_samples)
    predictions = apply_tree(tree, dataset)
    paths = get_path_in_tree(tree, dataset)
    println("Labeling instances...")
    ...
    originals = Vector{Vector{Float64}}(undef, _n_samples)
    samplerates = Vector{Number}(undef, _n_samples)
    ...
    filtered = Vector{Vector{Float64}}(undef, _n_samples)
    ...
    Threads.@threads for i in 1:_n_samples
        ...
        _n_features = length(results[i].path)
        bands = Vector{Int64}(undef, _n_features)
        weights = Vector{AbstractFloat}(undef, _n_features)
        for j in 1:_n_features
            if mode == :bandpass
                weights[j] = 1.0
    ...
    heatmap_png_path = Vector{String}(undef, _n_samples)
    ...
    Threads.@threads for i in 1:_n_samples
        ...
        if generate_json
            if !haskey(json_archive, string(results[i].predicted))
                json_archive[string(results[i].predicted)] = []
            end
            Threads.lock(json_lock)
            push!(json_archive[string(results[i].predicted)], filtered_file_path)
    ...
    spectrograms = Vector{NamedTuple{(:original, :filtered), Tuple{Plots.Plot, Plots.Plot}}}(undef, _n_samples)
    ...
    if draw_worlds_on_file
        ...
        for i in 1:_n_samples
            ...
            plot(plts..., layout = (_n_features, 1), size = (1000, 150 * _n_features))
    ...
    single_band_tracks = Vector{Vector{Tuple{Int64,Vector{Float64}}}}(undef, _n_samples)
