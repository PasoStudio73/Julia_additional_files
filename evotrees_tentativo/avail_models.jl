...
AVAIL_MODELS = Dict(
    ...
    :evotree_classifier => (
        ...
        ranges = [
            model -> MLJ.range(model, :lambda, lower=0, upper=1),
            model -> MLJ.range(model, :eta, lower=0, upper=1),
        ],

        rules_method = Sole.listrules
    ),

    :modal_evotree_classifier => (
        method = ModalEvoTreeClassifier,
        ...
        model_params = (;
            ...
            alpha=0.5, 
            tree_type="binary", 
            rng=Random.TaskLocalRNG(),  # common

            # modal decision tree extra params
            # max_depth=nothing,
            min_samples_leaf=4, 
            min_purity_increase=0.002, 
            max_purity_at_leaf=Inf, 
