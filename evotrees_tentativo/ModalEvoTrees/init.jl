function modal_init_core(params::ModalEvoTypes{L}, ::Type{CPU}, X, fnames, y_train, w, offset) where {L}

    # binarize X into quantiles
    edges, featbins, feattypes = modal_getedges(X; fnames, nbins=params.nbins, rng=params.rng)
    x_bin = modal_binarize(X; fnames, edges)
    ...
    elseif L in [Poisson, Gamma, Tweedie] # TODO
        @assert eltype(y_train) <: Real
        K = 1
        y = T.(y_train)
        μ = fill(log(mean(y)), 1)
        !isnothing(offset) && (offset .= log.(offset))
    elseif L == ModalMLogLoss
        ...
        μ .-= maximum(μ)
        !isnothing(offset) && (offset .= log.(offset))
    elseif L == GaussianMLE # TODO
        @assert eltype(y_train) <: Real
        K = 2
        y = T.(y_train)
        μ = [mean(y), log(std(y))]
        !isnothing(offset) && (offset[:, 2] .= log.(offset[:, 2]))
    elseif L == LogisticMLE # TODO
        @assert eltype(y_train) <: Real
        K = 2
        y = T.(y_train)
    ...
    # initialize model
    nodes = [Evo.TrainNode(featbins, K, view(is_in, 1:0)) for n = 1:2^params.max_depth-1]
    bias = [Evo.Tree{L,K}(μ)]
    m = Evo.EvoTree{L,K}(bias, info)
