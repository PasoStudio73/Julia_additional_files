...
# """
function modal_getedges(X::AbstractMatrix{T}; fnames, nbins, rng=Random.MersenneTwister()) where {T}
    @assert T <: Union{Real, Vector{<:Real}}
    nobs = min(size(X, 1), 1000 * nbins)
    idx = sample(rng, 1:size(X, 1), nobs, replace=false, ordered=true)
    nfeats = size(X, 2)
    edges = Vector{T}(undef, nfeats)
    featbins = Vector{UInt8}(undef, nfeats)
    feattypes = Vector{Bool}(undef, nfeats)
    @threads for j in 1:nfeats
        edges[j] = quantile(hcat(view(X, idx, j)...), (1:nbins-1) / nbins)
        if length(edges[j]) == 1
...
modal_getedges(df::AbstractDataFrame; kwargs...) = modal_getedges(Matrix(df); kwargs...)
...
"""
# function binarize(X::Union{AbstractMatrix, AbstractDataFrame}; edges)
...
function modal_binarize(df::AbstractDataFrame; fnames, edges)
    nobs = length(Tables.getcolumn(df, 1))
    x_bin = zeros(UInt8, nobs, length(fnames))
    @threads for j in eachindex(fnames)
        col = df[:, fnames[j]]
        if eltype(col) <: Vector{<:Bool} # TODO
            x_bin[:, j] .= col .+ 1
        elseif eltype(col) <: Vector{<:CategoricalValue} #TODO
            x_bin[:, j] .= levelcode.(col)
        elseif eltype(col) <: Vector{<:Real}
            # x_bin[:, j] .= searchsortedfirst.(Ref(edges[j]), col)
            x_bin[:, j] .= [searchsortedfirst(edges[j], mean(c)) for c in col]
        else
            @error "Invalid feature eltype: $(fnames[j]) is $(eltype(col))"
        end
    end
